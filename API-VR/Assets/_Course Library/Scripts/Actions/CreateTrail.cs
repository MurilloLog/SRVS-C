...
public class CreateTrail : MonoBehaviour
{
    ...
    private void HandleNetworkDrawing(string message)
    {
        try
        {
            TrailSerializer.Instance.GenerateTrailFromJSON(message);
            //Debug.Log("Received network drawing: " + message);
        }
        catch (Exception e)
        {
    ...
    {
        if (currentTrail && currentTrailRenderer != null)// && eventsManager != null)
        {
            ...
            Drawings drawingData = new Drawings(
                "",//eventsManager.id,
                "",//eventsManager.roomId,
                anchorID,
                trailPositions.Length > 0 ? trailPositions[0] : Vector3.zero,
                new List<Vector3>(trailPositions),
                lineColor,
                sizeLine
            );
            ...
            else
            {
                ...
                CommunicationManager.Instance.PublishMessage(DrawingTopic, jsonData);
